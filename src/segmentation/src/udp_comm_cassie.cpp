...
{
  sub_pos_ = nh_.subscribe<geometry_msgs::Vector3>("/ball_location", 200, &UdpCommunicator::callbackPos, this);
...
void UdpCommunicator::callbackPos(const geometry_msgs::Vector3::ConstPtr& msg)
{
  tst_mon_.linear.x = msg->Vector3.x;
  tst_mon_.linear.y = msg->Vector3.y;
  tst_mon_.linear.z = msg->Vector3.z;
}
